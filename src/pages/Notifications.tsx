    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-2">
          ...
          {unreadCount > 0 && (
            <span className="px-2 py-0.5 rounded-full bg-destructive text-destructive-foreground text-xs font-medium">{unreadCount} new</span>
          )}
        </div>
        {unreadCount > 0 && (
            ...
            <div className="divide-y divide-border">
              {notifications.map((n, i) => (
                <button
                  ...
                  className={cn(
                    "w-full text-left p-4 flex items-start gap-3 hover:bg-muted/50 transition-colors animate-fade-in",
                    !n.read && "bg-primary/5"
                  )}
